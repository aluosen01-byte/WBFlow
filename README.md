...
3. **类目自动提取与匹配**：类目来源优先级 JSON-LD 面包屑 → DOM 面包屑导航 → __NUXT__ 父/子类目名；按**源类目学习映射**自动选中 WB 父级/子类目（首次手动选择后自动记忆，同源类目下次自动匹配）；无映射时回退到上次使用的类目
4. **尺寸与重量自动提取**：从 __NUXT__ dimensions → JSON-LD additionalProperty → DOM 特性区块提取长/宽/高（cm）与重量（kg），弹窗内可编辑并随搬品提交为包装尺寸
5. **品牌非必填**：留空即可（实测 WB 允许空品牌建卡）
6. **定价库存**：售价自动带出源商品价格（手动按汇率调整），**仓库默认选中"我的仓库"**（无则选第一个），库存一键配置
7. **后端自动启动**：Native Messaging Host 模式，打开扩展设置或商品页时若后端未运行自动在后台拉起，免手动 npm start
8. **执行搬品**：实时显示六步进度，成功后给出 nmID 与卖家后台链接
