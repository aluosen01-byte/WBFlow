...
3. **类目与搬品保持一致**：源商品包屑提取类目名，按**源类目学习映射**自动选中 WB 父级/子类目（首次手动选择后自动记忆，同源类目下次自动匹配）；无映射时回退到上次使用的类目
4. **品牌非必填**：留空即可（实测 WB 允许空品牌建卡）
5. **定价库存**：售价自动带出源商品价格（手动按汇率调整），**仓库默认选中"我的仓库"**（无则选第一个），库存一键配置
6. **执行搬品**：实时显示六步进度，成功后给出 nmID 与卖家后台链接
...
6. 点击扩展图标打开设置弹窗：
   - 后端地址默认 `http://localhost:3000`（需先运行 `npm start`）
   - 团队共用同一后端时，改为后端服务器地址（如 `http://192.168.x.x:3000` 或公网域名）
   - 可配置默认仓库、价格策略、默认库存
7. 打开任意 WB 商品详情页，页面右下角出现「一键搬品」悬浮按钮
...
│   ├── manifest.json       #   MV3 清单（匹配 wildberries.ru/.cn 全站，页内判断商品页）
