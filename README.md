...
3. **选择类目与特性**：父类目、子类目级联，**自动记忆并选中上次使用的类目**（可在弹窗中修改）；公共特性（颜色/材质/性别/产地等）按源属性自动预填；重量/尺寸类特性自动映射为包装重量(kg)
4. **定价库存**：售价自动带出源商品价格（手动按汇率调整），**仓库默认选中"我的仓库"**（无则选第一个），库存一键配置
5. **执行搬品**：实时显示六步进度，成功后给出 nmID 与卖家后台链接
